...
PSL_TYPES = {'Matches' : int , 'Mismatches': int , 'RepMatch': int ,
             'Ns': int , 'QGapCount': int , 'QGapBases': int ,
             'TGapCount': int , 'TGapBases': int , 'Strand': str,
             'qName': str, 'qSize': int, 'qStart': int, 'qEnd': int,
             'tName': str, 'tSize': int, 'tStart': int, 'tEnd': int,
             'blockCount': int,
             'blockSizes': str, 'qStarts': str, 'tStarts': str}

PSL_LABELS_PRESIDENT = ['Matches', 'Mismatches', 'RepMatch', 'Ns', 'QGapCount',
                        'QGapBases', 'TGapCount', 'TGapBases', 'Strand',
                        'QName', 'QSize', 'QStart', 'QEnd', 'TName', 'TSize',
                        'TStart', 'TEnd', 'BlockCount', 'BlockSizes',
                        'QStarts', 'TStarts']
...
d_PSL_prescorr = dict([("PSL_"+PSL_LABELS_PRESIDENT[i], PSL_LABELS[pl_lower.index(x)])
...
def readPSL(filein, index="qName"):
    ...
    """
    df_PSL = pd.read_table(filein, sep="\t", header=None,
                           names=PSL_LABELS,
                           skiprows=5, index_col=index)
...
def readPSLfromPresident(filein, index="qName", PSL_prefix = "PSL_"):
    ...
    with the PSL prefix
    """
    df_PSL_president = pd.read_table(filein, sep = "\t", header=0,
                                     usecols=d_PSL_prescorr.keys())
...
def PSLpretty(target, query, PSLrow):
    ...
    if t_bloc_end != PSLrow.tEnd:
        print(f"Error the end of target is different from field: {t_bloc_end} and {PSLrow.tEnd}")
    if q_bloc_end != PSLrow.qEnd:
        print(f"Error the end of target is different from field: {q_bloc_end} and {PSLrow.qEnd}")
    # Use that for check?
...
def main():
    ...
    df_PSL = pd.read_table("psl_tests/pblat_row_test.tsv",
                            sep="\t" , header=None)
